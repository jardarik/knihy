...
$sql = "SELECT isbn, first_name, second_name, book_name, book_desc FROM knihy";
$result = $conn->query($sql);

// Výpis tabulky s knihami
if ($result && $result->num_rows > 0) { ?>
            <table class='table table-striped table-bordered'>
            <thead class='table-dark'><tr><th>ISBN</th><th>Autor</th><th>Název knihy</th><th>Popis knihy</th></tr></thead><tbody> 
            <?php
    while ($row = $result->fetch_assoc()) {
        echo "<tr>";
        echo "<td>" . htmlspecialchars($row["isbn"]) . "</td>";
        echo "<td>" . htmlspecialchars($row["first_name"]) . " " . htmlspecialchars($row["second_name"]) . "</td>";
